...
const CountryList = () => {
  ...
  const filtered   = items.filter(i =>
    !search ||
    i.country_name.toLowerCase().includes(search.toLowerCase()) ||
    (i.currency_code ?? '').toLowerCase().includes(search.toLowerCase())
  );
  ...
  const validate = () => {
    ...
    if (!form.country_name.trim()) errs.country_name = 'Country Name is required.';
    if (items.some(i => i.country_name.toLowerCase() === form.country_name.trim().toLowerCase() && i.id !== editing?.id))
      errs.country_name = 'Country Name already exists.';
    return errs;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updateCountryApi(editing.id, payload);
        toast({ title: 'Success', description: 'Country updated successfully.', variant: 'success' });
      } else {
        await createCountryApi(payload);
        toast({ title: 'Success', description: 'Country created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: 'Success', description: 'Country deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Country</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage country configurations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['SR NO', 'COUNTRY NAME', 'CURRENCY CODE', 'SYMBOL', 'DESCRIPTION', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-muted-foreground text-sm">No countries found.</td></tr>
              ) : paginated.map((item, idx) => (
                <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                  <td className="p-4 text-sm text-muted-foreground">{(page - 1) * PAGE_SIZE + idx + 1}</td>
                  <td className="p-4 text-sm font-medium text-foreground whitespace-nowrap">{item.country_name}</td>
                  <td className="p-4 text-sm font-mono font-semibold text-foreground">{item.currency_code || '—'}</td>
                  <td className="p-4 text-sm text-foreground">{item.symbol || '—'}</td>
                  <td className="p-4 text-sm text-muted-foreground max-w-[160px] truncate" title={item.description}>{item.description || '—'}</td>
                  <td className="p-4">
