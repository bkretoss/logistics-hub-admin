...
import {
  getPricingTeamApi,
  createPricingTeamApi,
  updatePricingTeamApi,
  deletePricingTeamApi,
  getCompaniesApi,
} from '@/services/api';

interface Company { id: number; name: string; }

interface PricingTeamMember {
  id: number;
  name: string;
  email: string;
  company_id: number;
...
const PER_PAGE = 10;
const emptyForm = { name: '', email: '', company_id: '', description: '', status: 'active' as 'active' | 'inactive' };
...
const PricingTeamList = () => {
  const { toast } = useToast();
  const [allItems, setAllItems]         = useState<PricingTeamMember[]>([]);
  ...
  const [editing, setEditing]           = useState<PricingTeamMember | null>(null);
  const [viewItem, setViewItem]         = useState<PricingTeamMember | null>(null);
  ...
  const loadCompanies = async () => {
    try {
      ...
      setCompanies(raw.map(r => ({ id: r.id, name: r.name })));
    } catch {}
  };
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getPricingTeamApi(1, 9999);
      const raw: any[] = res.data?.data ?? res.data ?? [];
      ...
      toast({ title: 'Error', description: 'Failed to load pricing team members.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const filtered = allItems.filter(i => {
    ...
    const matchCompany = companyFilter === 'all' || String(i.company_id) === companyFilter;
  ...
  const openEdit   = (item: PricingTeamMember) => {
    setEditing(item);
    setForm({ name: item.name, email: item.email, company_id: String(item.company_id), description: item.description, status: item.status });
    setErrors({});
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updatePricingTeamApi(editing.id, payload);
        toast({ title: 'Success', description: 'Pricing team member updated successfully.', variant: 'success' });
      } else {
        await createPricingTeamApi(payload);
        toast({ title: 'Success', description: 'Pricing team member created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch (err: any) {
      ...
      if (/duplicate|unique|already exists/i.test(msg)) {
        ...
        toast({ title: 'Error', description: 'Failed to save pricing team member.', variant: 'destructive' });
      }
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      await deletePricingTeamApi(deleteId);
      toast({ title: 'Success', description: 'Pricing team member deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
      toast({ title: 'Error', description: 'Failed to delete pricing team member.', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Pricing Team</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage pricing team members</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['SR NO', 'NAME', 'EMAIL', 'COMPANY', 'DESCRIPTION', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-muted-foreground text-sm">No pricing team members found.</td></tr>
                ...
                <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-foreground whitespace-nowrap">{getCompanyName(item.company_id)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit Pricing Team Member' : 'Add Pricing Team Member'}</h3>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  <Label className="text-sm font-semibold text-right w-32 shrink-0">
                    <span className="text-destructive mr-1">*</span>Company
                  </Label>
                  <div className="flex-1">
                    <select
                      value={form.company_id}
                      onChange={e => { setForm(f => ({ ...f, company_id: e.target.value })); setErrors(er => ({ ...er, company_id: '' })); }}
                      className={`w-full px-3 py-2 border rounded-lg text-sm bg-background ${errors.company_id ? 'border-destructive' : 'border-input'}`}
                    >
      ...
      {viewItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Pricing Team Member</h3>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                {[
                  ...
                  { label: 'Company',      value: getCompanyName(viewItem.company_id) },
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Pricing Team Member</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this member? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
...
export default PricingTeamList;
