...
const ProspectMasterList = () => {
  const { toast } = useToast();
  const [allItems, setAllItems]         = useState<Prospect[]>([]);
  ...
  const [viewItem, setViewItem]         = useState<Prospect | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getProspectsApi(1, 9999);
  ...
  const openCreate = () => { setEditing(null); setForm(emptyForm); setNameError(''); setModalOpen(true); };
  const openEdit   = (item: Prospect) => {
    setEditing(item);
  ...
  const isDuplicate = (name: string, excludeId?: number) =>
    allItems.some(i => i.name.trim().toLowerCase() === name.trim().toLowerCase() && i.id !== excludeId);

  const handleSave = async () => {
    const trimmedName = form.name.trim();
    if (!trimmedName) { setNameError('Prospect Name is required.'); return; }
    if (isDuplicate(trimmedName, editing?.id)) {
      setNameError('Prospect Name already exists. Please enter a unique name.');
      return;
    }
    setSaving(true);
    try {
      ...
      if (editing) {
        await updateProspectApi(editing.id, payload);
        toast({ title: 'Success', description: 'Prospect updated successfully.', variant: 'success' });
      } else {
        await createProspectApi(payload);
        toast({ title: 'Success', description: 'Prospect created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteProspectApi(deleteId);
      toast({ title: 'Success', description: 'Prospect deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Manage prospect configurations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['SR NO', 'NAME', 'DESCRIPTION', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-muted-foreground text-sm">No prospects found.</td></tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit Prospect' : 'Add Prospect'}</h3>
      ...
      {viewItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Prospect</h3>
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Prospect</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this prospect? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
...
export default ProspectMasterList;
