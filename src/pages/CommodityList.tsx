...
const PER_PAGE = 10;
const emptyForm = { name: '', code: '', hs_code: '', description: '', status: 'active' as 'active' | 'inactive' };
...
const CommodityList = () => {
  ...
  const [allItems, setAllItems]         = useState<Commodity[]>([]);
  ...
  const [viewItem, setViewItem]         = useState<Commodity | null>(null);
  const [form, setForm]                 = useState(emptyForm);
  const [errors, setErrors]             = useState({ name: '', code: '', hs_code: '' });
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getCommoditiesApi(1, 9999);
      const raw: any[] = res.data?.data ?? res.data ?? [];
      setAllItems(raw.map(r => ({ ...r, status: r.status === 1 || r.status === '1' ? 'active' : 'inactive' })));
    } catch {
      toast({ title: 'Error', description: 'Failed to load commodities.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const openCreate = () => { setEditing(null); setForm(emptyForm); setErrors({ name: '', code: '', hs_code: '' }); setModalOpen(true); };
  const openEdit   = (item: Commodity) => {
    setEditing(item);
    setForm({ name: item.name, code: item.code, hs_code: item.hs_code, description: item.description, status: item.status });
    setErrors({ name: '', code: '', hs_code: '' });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    const newErrors = { name: '', code: '', hs_code: '' };
    ...
    if (form.hs_code && form.hs_code.length > 100) newErrors.hs_code = 'HS Code must not exceed 100 characters.';
    if (newErrors.name || newErrors.code || newErrors.hs_code) { setErrors(newErrors); return; }

    setSaving(true);
    try {
      const payload = { name: trimmedName, code: trimmedCode, hs_code: form.hs_code, description: form.description, status: form.status === 'active' ? '1' : '0' };
      if (editing) {
        await updateCommodityApi(editing.id, payload);
        toast({ title: 'Success', description: 'Commodity updated successfully.', variant: 'success' });
      } else {
        await createCommodityApi(payload);
        toast({ title: 'Success', description: 'Commodity created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch (err: any) {
      ...
      if (/duplicate|unique|already exists/i.test(msg)) {
        ...
        toast({ title: 'Error', description: msg || 'Failed to save commodity.', variant: 'destructive' });
      }
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteCommodityApi(deleteId);
      toast({ title: 'Success', description: 'Commodity deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
  ...
  const handleStatusToggle = async (item: Commodity) => {
    try {
      await updateCommodityStatusApi(item.id, item.status === 'active' ? 0 : 1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Commodity</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage commodity configurations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['SR NO', 'NAME', 'CODE', 'HS CODE', 'DESCRIPTION', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-muted-foreground text-sm">No commodities found.</td></tr>
                ...
                <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-muted-foreground whitespace-nowrap">{item.hs_code || '—'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit Commodity' : 'Add Commodity'}</h3>
      ...
      {viewItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Commodity</h3>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                {[
                  ...
                  { label: 'HS Code',      value: viewItem.hs_code || '—' },
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Commodity</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this commodity? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
