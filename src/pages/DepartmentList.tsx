...
const DepartmentList = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getDepartmentsApi(1, 9999);
  ...
  // Client-side filter — same approach as CargoTypeList
  ...
  const handleSave = async () => {
    ...
    if (!trimmedName) { setNameError('Department Name is required.'); return; }
    if (isDuplicate(trimmedName, editing?.id)) {
      setNameError('Department Name already exists. Please enter a unique name.');
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Department</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage department configurations</p>
        </div>
      ...
      {/* Stats — always based on full allItems, never affected by filters */}
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-muted-foreground text-sm">No departments found.</td></tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit Department' : 'Add Department'}</h3>
      ...
      {viewItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Department</h3>
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Department</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this department? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
