...
import {
  ...
  createTransportModeApi,
  updateTransportModeApi,
  deleteTransportModeApi,
} from '@/services/api';
...
const TransportModeList = () => {
  ...
  const [viewItem, setViewItem]   = useState<TransportMode | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Failed to load transport modes.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const openEdit   = (item: TransportMode) => {
    setEditing(item);
  ...
  const handleSave = async () => {
    if (!form.name.trim()) { setFormError('Transport Mode Name is required.'); return; }
    if (items.some(i => i.name.toLowerCase() === form.name.trim().toLowerCase() && i.id !== editing?.id)) {
      setFormError('Transport Mode Name already exists.'); return;
    }
    setSaving(true);
    try {
      const payload = { ...form, status: form.status === 'Active' ? 1 : 0 };
      if (editing) {
        await updateTransportModeApi(editing.id, payload);
        toast({ title: 'Success', description: 'Transport mode updated successfully.', variant: 'success' });
      } else {
        await createTransportModeApi(payload);
        toast({ title: 'Success', description: 'Transport mode created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch {
      toast({ title: 'Error', description: 'Failed to save transport mode.', variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTransportModeApi(deleteId);
      toast({ title: 'Success', description: 'Transport mode deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
      toast({ title: 'Error', description: 'Failed to delete transport mode.', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Transport Mode Type</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage transport mode configurations</p>
        </div>
        <Button className="gap-2 material-button material-elevation-2 hover:material-elevation-3 text-black" onClick={openCreate}>
          <Plus className="w-4 h-4" /> Add Transport Mode
        </Button>
      </div>
      ...
      <div className="material-card material-elevation-1 p-4">
        <div className="relative max-w-md">
          ...
          <input
            type="text"
            placeholder="Search by transport mode name..."
            value={search}
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['ID', 'TRANSPORT MODE NAME', 'DESCRIPTION', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} className="p-8 text-center text-muted-foreground text-sm">No transport modes found.</td></tr>
              ) : paginated.map(item => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Transport Mode</h3>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit Transport Mode' : 'Add Transport Mode'}</h3>
              <button onClick={() => setModalOpen(false)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Label className="text-sm font-semibold text-right w-32 shrink-0"><span className="text-destructive mr-1">*</span>Name</Label>
                  <div className="flex-1">
                    <input
                      type="text"
                      value={form.name}
                      onChange={e => { setForm(f => ({ ...f, name: e.target.value })); setFormError(''); }}
                      placeholder="e.g. Sea, Air, Road, Rail"
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Transport Mode</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this transport mode? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
