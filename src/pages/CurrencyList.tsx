...
const normalizeStatus = (s: number | string): 'Active' | 'Inactive' =>
  s === 1 || s === 'active' || s === 'Active' ? 'Active' : 'Inactive';
...
const PAGE_SIZE = 10;
const emptyForm = { name: '', code: '', symbol: '', status: 'Active' as 'Active' | 'Inactive' };
...
const CurrencyList = () => {
  ...
  const [viewItem, setViewItem]     = useState<Currency | null>(null);
  ...
  const openEdit   = (item: Currency) => {
    setEditing(item);
    setForm({ name: item.name, code: item.code, symbol: item.symbol, status: item.status });
    setFormErrors({});
    setModalOpen(true);
  };

  const handleFieldChange = (name: keyof FormType, value: string) => {
  ...
  const validate = () => {
    ...
    if (!form.symbol.trim()) errs.symbol = 'Currency symbol is required.';
    if (items.some(i => i.code.toLowerCase() === form.code.trim().toLowerCase() && i.id !== editing?.id))
      errs.code = 'Currency code already exists.';
    return errs;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updateCurrencyApi(editing.id, payload);
        toast({ title: 'Success', description: 'Currency updated successfully.', variant: 'success' });
      } else {
        await createCurrencyApi(payload);
        toast({ title: 'Success', description: 'Currency created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch {
      toast({ title: 'Error', description: 'Failed to save currency.', variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteCurrencyApi(deleteId);
      toast({ title: 'Success', description: 'Currency deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Currency</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage currency configurations</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['ID', 'CURRENCY NAME', 'CODE', 'SYMBOL', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="p-8 text-center text-muted-foreground text-sm">No currencies found.</td></tr>
              ) : paginated.map(item => (
                <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-4 text-sm font-mono font-semibold text-foreground">{item.code}</td>
                  <td className="p-4 text-sm text-foreground">{item.symbol}</td>
                  <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Currency</h3>
              <button onClick={() => setViewItem(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                {[
                  { label: 'Currency Name',   value: viewItem.name },
                  { label: 'Currency Code',   value: viewItem.code },
                  { label: 'Currency Symbol', value: viewItem.symbol },
                  { label: 'Created Date',    value: formatDate(viewItem.created_at) },
                  { label: 'Updated Date',    value: formatDate(viewItem.updated_at) },
                ].map(({ label, value }) => (
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit Currency' : 'Add Currency'}</h3>
              <button onClick={() => setModalOpen(false)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <Label className="text-sm font-semibold text-right w-32 shrink-0"><span className="text-destructive mr-1">*</span>Currency Name</Label>
                ...
                <div className="flex items-center gap-3">
                  <Label className="text-sm font-semibold text-right w-32 shrink-0"><span className="text-destructive mr-1">*</span>Currency Code</Label>
                  <div className="flex-1">
                    <input
                      type="text"
                      value={form.code}
                      ...
                      placeholder="e.g. USD"
                      className={`w-full px-3 py-2 border rounded-lg text-sm bg-background ${
                        formErrors.code ? 'border-destructive' : 'border-input'
                      }`}
                    />
                ...
                <div className="flex items-center gap-3">
                  <Label className="text-sm font-semibold text-right w-32 shrink-0"><span className="text-destructive mr-1">*</span>Symbol</Label>
                  <div className="flex-1">
                    <input
                      type="text"
                      value={form.symbol}
                      ...
                      placeholder="e.g. $"
                      className={`w-full px-3 py-2 border rounded-lg text-sm bg-background ${
                        formErrors.symbol ? 'border-destructive' : 'border-input'
                      }`}
                    />
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Currency</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this currency? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
