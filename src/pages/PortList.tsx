...
import { getMasterPortsApi, deleteMasterPortApi, getMasterPortApi } from '@/services/api';
...
const PortList = () => {
  ...
  const [allItems, setAllItems]   = useState<Port[]>([]);
  const [loading, setLoading]     = useState(false);
  const [search, setSearch]       = useState('');
  const [page, setPage]           = useState(1);
  const [viewItem, setViewItem]   = useState<Port | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await getMasterPortsApi(1, 9999);
      const raw: any[] = res.data?.data ?? res.data ?? [];
      // Fetch each port's detail to get terminals
      const detailed = await Promise.all(
        raw.map(p => getMasterPortApi(p.id).then(r => r.data?.data ?? r.data).catch(() => p))
      );
      setAllItems(detailed);
    } catch {
      toast({ title: 'Error', description: 'Failed to load ports.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleView = async (item: Port) => {
    setViewLoading(true);
    setViewItem(item);
    try {
      const res = await getMasterPortApi(item.id);
      setViewItem(res.data?.data ?? res.data);
    } catch {
      // keep basic item
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteMasterPortApi(deleteId);
      toast({ title: 'Success', description: 'Port deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
      toast({ title: 'Error', description: 'Failed to delete port.', variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">Port Master</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage port configurations</p>
        </div>
        <Button className="gap-2 material-button material-elevation-2 hover:material-elevation-3 text-black" onClick={() => navigate('/master/port/new')}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          { label: 'ACTIVE',   value: allItems.filter(i => i.status === 1).length,        color: 'text-green-500', bg: 'bg-green-50' },
          { label: 'INACTIVE', value: allItems.filter(i => i.status !== 1).length,        color: 'text-red-500',   bg: 'bg-red-50'   },
        ].map(s => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['SR NO', 'NAME', 'CODE', 'DESCRIPTION', 'TERMINALS', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-muted-foreground text-sm">No ports found.</td></tr>
                ...
                <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-muted-foreground">{item.code}</td>
                  <td className="p-4 text-sm text-muted-foreground max-w-[180px] truncate" title={item.description}>{item.description || '—'}</td>
                  <td className="p-4 text-sm text-muted-foreground">
                    {item.terminals && item.terminals.length > 0
                      ? <div className="flex flex-wrap gap-1">
                          {item.terminals.map((t, i) => (
                            <span key={i} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 whitespace-nowrap">{t.name}</span>
                          ))}
                        </div>
                      : '—'
                    }
                  ...
                  <td className="p-4">
                    <div className="flex items-center justify-end gap-2">
                      <button className="p-1.5 hover:bg-blue-50 rounded-lg transition-colors" title="View" onClick={() => handleView(item)}>
                        <Eye className="w-4 h-4 text-blue-500" />
                      </button>
                      <button className="p-1.5 hover:bg-green-50 rounded-lg transition-colors" title="Edit" onClick={() => navigate(`/master/port/edit/${item.id}`)}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View Port</h3>
              ...
              {viewLoading ? (
                ...
                <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                      viewItem.status === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                      {viewItem.status === 1 ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                  {viewItem.terminals && viewItem.terminals.length > 0 && (
                    <div>
                      <Label className="text-sm font-semibold block mb-2">Terminals</Label>
                              ...
                              <tr key={i} className="border-t border-border">
                                ...
                                <td className="p-2">
                                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${
                                    t.status === 1 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                  }`}>
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete Port</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this port? This action cannot be undone.</p>
            </div>
