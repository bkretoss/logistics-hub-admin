...
import {
  getCitiesApi, createCityApi, updateCityApi, deleteCityApi,
  getCountriesApi, getStatesApi,
} from '@/services/api';

interface Country { id: number; name: string; }
interface StateItem { id: number; country_id: number; name: string; }
interface City {
  id: number;
  country_id: number;
  state_id: number;
...
const PER_PAGE = 10;
const emptyForm = { country_id: '', state_id: '', name: '', description: '', status: 'active' as 'active' | 'inactive' };
...
const CityList = () => {
  ...
  // States filtered by selected country in form
  const formStates = allStates.filter(s => !form.country_id || String(s.country_id) === form.country_id);
  // States filtered by selected country in toolbar filter
  const filterStates = countryFilter === 'all'
    ? allStates
    : allStates.filter(s => s.country_id === Number(countryFilter));

  const getCountryName = (id: number) => countries.find(c => c.id === id)?.name ?? '—';
  const getStateName   = (id: number) => allStates.find(s => s.id === id)?.name ?? '—';
  ...
  const loadCountries = async () => {
    try {
      ...
      const raw: any[] = res.data?.data ?? res.data ?? [];
      setCountries(raw.filter(r => r.status === 1 || r.status === '1').map(r => ({ id: r.id, name: r.country_name })));
    } catch {}
  };
  ...
  useEffect(() => { load(); loadCountries(); loadStates(); }, []);

  const filtered = allItems.filter(i => {
    const matchSearch  = !search || i.name.toLowerCase().includes(search.toLowerCase());
    const matchStatus  = statusFilter === 'all' || i.status === statusFilter;
    const matchCountry = countryFilter === 'all' || String(i.country_id) === countryFilter;
    const matchState   = stateFilter === 'all' || String(i.state_id) === stateFilter;
  ...
  const openEdit   = (item: City) => {
    setEditing(item);
    setForm({ country_id: String(item.country_id), state_id: String(item.state_id), name: item.name, description: item.description, status: item.status });
    setErrors({});
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updateCityApi(editing.id, payload);
        toast({ title: 'Success', description: 'City updated successfully.', variant: 'success' });
      } else {
        await createCityApi(payload);
        toast({ title: 'Success', description: 'City created successfully.', variant: 'success' });
      }
      setModalOpen(false);
      load();
    } catch (err: any) {
      ...
      if (/duplicate|unique|already exists/i.test(msg)) {
        setErrors(e => ({ ...e, name: 'City Name already exists.' }));
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteCityApi(deleteId);
      toast({ title: 'Success', description: 'City deleted successfully.', variant: 'success' });
      setDeleteId(null);
      load();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground tracking-tight">City</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage city configurations</p>
        </div>
      ...
      <div className="material-card material-elevation-1 p-4">
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select
            value={countryFilter}
            onChange={e => { setCountryFilter(e.target.value); setStateFilter('all'); setPage(1); }}
            className="w-44 px-3 py-2.5 border border-input rounded-xl text-sm bg-background"
          >
          ...
          <select
            ...
            className="w-44 px-3 py-2.5 border border-input rounded-xl text-sm bg-background disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <option value="all">{countryFilter === 'all' ? 'Select States' : 'All States'}</option>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                {['SR NO', 'COUNTRY', 'STATE', 'CITY NAME', 'DESCRIPTION', 'STATUS', 'CREATED DATE', 'ACTIONS'].map((h, i) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="p-8 text-center text-muted-foreground text-sm">No cities found.</td></tr>
                ...
                <tr key={item.id} className="border-b border-border hover:bg-muted/50 transition-colors">
                  ...
                  <td className="p-4 text-sm text-foreground whitespace-nowrap">{getStateName(item.state_id)}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">{editing ? 'Edit City' : 'Add City'}</h3>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <select
                      ...
                      onChange={e => {
                        setForm(f => ({ ...f, country_id: e.target.value, state_id: '' }));
                        setErrors(er => ({ ...er, country_id: '' }));
                      }}
                ...
                <div className="flex items-center gap-3">
                  <Label className="text-sm font-semibold text-right w-32 shrink-0">
                    <span className="text-destructive mr-1">*</span>State
                  </Label>
                  <div className="flex-1">
                    <select
                      value={form.state_id}
                      onChange={e => { setForm(f => ({ ...f, state_id: e.target.value })); setErrors(er => ({ ...er, state_id: '' })); }}
                      disabled={!form.country_id}
                      className={`w-full px-3 py-2 border rounded-lg text-sm bg-background disabled:opacity-60 ${errors.state_id ? 'border-destructive' : 'border-input'}`}
                    >
                      <option value="">{form.country_id ? 'Select State' : 'Select States'}</option>
                      {formStates.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                    </select>
                    {errors.state_id && <p className="text-xs text-destructive mt-1">⚠ {errors.state_id}</p>}
                  </div>
                </div>
      ...
      {viewItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">View City</h3>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <div className="space-y-4">
                {[
                  ...
                  { label: 'State',        value: getStateName(viewItem.state_id) },
      ...
      {deleteId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background rounded-lg shadow-2xl w-full max-w-sm mx-4 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <h3 className="text-lg font-bold text-primary">Delete City</h3>
              <button onClick={() => setDeleteId(null)} className="p-2 hover:bg-muted rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-5">
              <p className="text-sm text-muted-foreground">Are you sure you want to delete this city? This action cannot be undone.</p>
            </div>
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-border shrink-0">
